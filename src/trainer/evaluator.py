...
class Evaluator:
    ...
    def eval(self, y_true, y_pred, valid_ids=None):
        ...
        elif self.eval_metric == 'rmse_split':
            return self._eval_rmse_split(y_true, y_pred)
    ...
    def _eval_rmse_split(self, y_true, y_pred):
        '''
            compute RMSE score averaged across tasks
        '''
        rmse_list = []
    ...
    def _eval_r(self, y_true, y_pred):
        '''
            compute R score averaged across tasks
        '''
        r_list = []
        ...
        for i in range(y_true.shape[1]):
            ...
            is_labeled = y_true[:, i] == y_true[:, i]
            if (self.mean is not None) and (self.std is not None):
                pearson_corr, _ = pearsonr(y_true[is_labeled, i], y_pred[is_labeled, i] * self.std[i] + self.mean[i])
                # results = np.stack([y_true[is_labeled, i], y_pred[is_labeled, i] * self.std[i] + self.mean[i]])
                # with open("../results/fig5/cnn_supervised.pkl", "wb") as f:
                #     pickle.dump(results, f)
            else:
                pearson_corr, _ = pearsonr(y_true[is_labeled, i], y_pred[is_labeled, i])
