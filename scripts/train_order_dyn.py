...
def main(args):
    ...
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.n_threads, worker_init_fn=seed_worker, generator=g, drop_last=True)

    model_cls = OrderModel
    loss_fn_clip = cliploss
    loss_fn_rnc = RnCLoss(train_dataset.mean, train_dataset.std, args.device, args.n_epochs, label_diff='l2', feature_sim='product')
